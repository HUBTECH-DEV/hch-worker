...
export const MANIFEST_CONTENT_COMPATIBILITY_BOOTSTRAP_VERSION = "2.3.0";
...
/**
 * Returns the hash of the signed manifest fields that can change generated
 * content. Runtime metadata, capacity, endpoints, validity windows, and
 * platform release details are intentionally outside this contract.
 *
 * This projection is shared with the orchestrator. Any change to it is a
 * public protocol change and must be coordinated before release.
 */
export async function manifestContentContractHash(manifest) {
  if (!isPlainObject(manifest)) {
    throw new TypeError("The manifest content contract must be an object.");
  }
  const editorial = isPlainObject(manifest.editorial) ? manifest.editorial : null;
  const engine = isPlainObject(manifest.engine) ? manifest.engine : null;
  const contract = {
    adaptiveWorkPolicy: manifest.adaptiveWorkPolicy ?? null,
    artifacts: manifest.artifacts ?? null,
    editorial: editorial && {
      pipelineVersion: editorial.pipelineVersion ?? null,
      policyHash: editorial.policyHash ?? null,
      promptConfigHash: editorial.promptConfigHash ?? null,
    },
    engine: engine && {
      adapter: engine.adapter ?? null,
      adapterVersion: engine.adapterVersion ?? null,
      model: engine.model ?? null,
      modelDigest: engine.modelDigest ?? null,
      protocol: engine.protocol ?? null,
  ...
  const digest = await subtle().digest(
    "SHA-256",
    encoder.encode(canonicalizeJson(contract)),
  );
  return bytesToHex(new Uint8Array(digest));
}

/**
 * Validates the self-declared compatibility metadata of a signed manifest.
 * Older bootstrap contracts remain valid without the declaration; bootstrap
 * 2.3.0 requires it and fails closed when its content projection is untrue.
 */
export async function verifyManifestContentCompatibility(manifest) {
  try {
    if (!isPlainObject(manifest)) {
      return failure(
        "manifest-content-contract-invalid",
        "The manifest content contract must be an object.",
      );
    }
    ...
    if (compatibility === undefined) {
      if (
        ...
        MANIFEST_CONTENT_COMPATIBILITY_BOOTSTRAP_VERSION
      ) {
        return failure(
      ...
      return { ok: true, compatibility: null };
    }
    if (!isPlainObject(compatibility)) {
      return failure(
        "manifest-compatibility-invalid",
        "The manifest compatibility declaration is invalid.",
      );
    }
    const classification = compatibility.classification;
    const contentImpact = compatibility.contentImpact;
    const currentHash = compatibility.contentContractHash;
    const previousHash = compatibility.previousContentContractHash;
    const minimumWorkerVersion = semanticVersion(
      compatibility.minimumWorkerVersion,
    );
    const testedThroughWorkerVersion = semanticVersion(
      compatibility.testedThroughWorkerVersion,
    );
    if (
      !new Set(["initial", "compatible", "content-incompatible"]).has(
        classification,
      ) ||
      !lowerSha256(currentHash) ||
      !(previousHash === null || lowerSha256(previousHash)) ||
      !minimumWorkerVersion ||
      !testedThroughWorkerVersion ||
      compareSemanticVersions(
        minimumWorkerVersion,
        testedThroughWorkerVersion,
      ) > 0 ||
      !new Set(["none", "generated-content"]).has(contentImpact) ||
      !validCompatibilityRelationship(
        classification,
        contentImpact,
        currentHash,
        previousHash,
    ...
    const calculatedHash = await manifestContentContractHash(manifest);
    if (!sameText(currentHash, calculatedHash)) {
      return failure(
    ...
    return {
      ...
      compatibility: {
        classification,
        contentContractHash: currentHash,
        previousContentContractHash: previousHash,
        minimumWorkerVersion: compatibility.minimumWorkerVersion,
        testedThroughWorkerVersion: compatibility.testedThroughWorkerVersion,
        contentImpact,
      },
    };
  } catch (error) {
    return failure("manifest-compatibility-invalid", safeErrorMessage(error));
  }
}
...
export async function verifyReleaseKeyDelegation(
  ...
  try {
    ...
    if (delegation.expires < now - skew && options.allowExpired !== true) {
...
export async function verifyManifestWithDelegation(
  ...
  const compatibilityResult = await verifyManifestContentCompatibility(
...
function validateTimeWindow(created, expires, options) {
  ...
  if (expires < now - skew && options.allowExpired !== true) {
...
function validCompatibilityRelationship(
  classification,
  contentImpact,
  currentHash,
  previousHash,
  ...
  if (classification === "initial") {
    return previousHash === null && contentImpact === "none";
  }
  if (classification === "compatible") {
    return previousHash === currentHash && contentImpact === "none";
  }
  return (
    classification === "content-incompatible" &&
    lowerSha256(previousHash) &&
